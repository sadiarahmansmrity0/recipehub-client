...
import { FaUtensils, FaUser, FaSignOutAlt, FaSignInAlt, FaUserPlus, FaBookOpen } from 'react-icons/fa';
...
export default function Navbar() {
  ...
  const handleLogout = async () => {
    try {
      // 1. Tell the server to delete the cookie/session
      await api.post('/auth/logout'); 

      // 2. Clear the user state in your React context
      setUser(null); 
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-orange-100/50 shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <Link href="/" className="text-gray-600 hover:text-orange-600 px-3 py-2 rounded-lg hover:bg-orange-50 transition-all duration-200">
              Home
            </Link>
      ...
      className="text-gray-600 hover:text-orange-600 px-3 py-2 rounded-lg hover:bg-orange-50 transition-all duration-200"
    >
      My Recipes
    </Link>
...
<Link href="/dashboard/profile" className="text-gray-600 hover:text-orange-600 px-3 py-2 rounded-lg hover:bg-orange-50 transition-all duration-200">
    Profile
</Link>
